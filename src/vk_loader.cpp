        ...
        fastgltf::visitor{
            ...
            [&](fastgltf::sources::Vector &vector)
            {
                unsigned char *data = stbi_load_from_memory(
                    reinterpret_cast<const unsigned char *>(vector.bytes.data()), static_cast<int>(vector.bytes.size()),
            ...
            [&](fastgltf::sources::BufferView &view)
            {
                ...
                std::visit(fastgltf::visitor{
                               ...
                               {
                                   unsigned char *data = stbi_load_from_memory(
                                       reinterpret_cast<const unsigned char *>(vector.bytes.data()) + bufferView.
                                       byteOffset,
                               ...
                               {
                                   unsigned char *data = stbi_load_from_memory(
                                       reinterpret_cast<const unsigned char *>(byteView.bytes.data()) + bufferView.
                                       byteOffset,
                                       static_cast<int>(bufferView.byteLength),
                                       &width, &height, &nrChannels, 4);
                                   if (data)
                               ...
                               {
                                   fmt::print("Debug: Buffer is Array (Size: {})\n", array.bytes.size());

                                   auto *rawBytes = reinterpret_cast<const unsigned char *>(array.bytes.data());

                                   if (bufferView.byteOffset + bufferView.byteLength > array.bytes.size())
                                   {
                                       fmt::print("Error: BufferView out of bounds\n");
                                       return;
                                   }

                                   unsigned char *data = stbi_load_from_memory(
                                       rawBytes + bufferView.byteOffset,
                                       static_cast<int>(bufferView.byteLength),
                                       &width, &height, &nrChannels, 4);
